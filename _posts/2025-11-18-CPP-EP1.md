...
> **Time:** 2025-11-18 23:25
> **Location:** 深圳大学致理楼L2-406
...
同时需要注意的是，当存在两个以上的matrix时，或名字不同时，solve函数内的就不可以直接锁定matrix了。
这时候变为：
```cpp
...
在c++里 `++i`比`i++`执行速度更快，因为`++i`是直接在i上进行操作，而`i++`则会先生成一个临时变量，对i进行加减后，这个临时变量没变，直接给扔了。步骤多了。
总之一般就是用`++i`。
